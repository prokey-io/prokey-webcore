...
export class BitcoinWallet extends BaseWallet {
    private _bitcoinWallet!: BaseWalletModel<BitcoinAccountInfo>
     ...
     */
    public async StartDiscovery(accountFindCallBack?: (accountInfo: WalletModel.BitcoinAccountInfo) => void, allAccounts = false): Promise<BaseWalletModel<BitcoinAccountInfo>>{
        this._bitcoinWallet = {
            totalBalance: 0,
        }

        this._bitcoinWallet.accounts = new Array<BitcoinAccountInfo>();

        return new Promise<BaseWalletModel<BitcoinAccountInfo>>(async (resolve, reject) => {
            try {
     ...
     */
    public GetWallet(): BaseWalletModel<BitcoinAccountInfo> {
