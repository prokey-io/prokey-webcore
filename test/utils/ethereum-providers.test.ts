import { GetGasParams, EstimateGasLimit } from '../../src/utils/ethereum-providers';
...
describe('ethereum-providers test', () => {
    describe('GetGasParams function test', () => {
        ...
        it('should get rejected when chain id not founded or not valid', async () => {
            await expect(GetGasParams(55)).to.be.rejectedWith("Couldn't find any provider with specified chain id.");
        });
        ...
        it('should get rejected when url is not valid or connection takes longer than 5 seconds', async () => {
            await expect(GetGasParams('http://AN-INVALID-IRL.org')).to.be.rejectedWith('Connection timeout');
        });
    });
    ...
    describe('EstimateGasLimit test', () => {
        ...
        it('should get rejected when chain id not founded or not valid', async () => {
            await expect(EstimateGasLimit(99, contractTransaction)).to.be.rejectedWith(
